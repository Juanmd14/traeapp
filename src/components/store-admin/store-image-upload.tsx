  ...
  const uploadImage = async (base64: string) => {
    ...
    try {
      const res = await fetch("/api/upload-store-image", {
        ...
        body: JSON.stringify({ storeId, imageBase64: base64, type }),
      });
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-body-xs text-neutral-500">
        {isLogo
          ? "Logo mostrado en la tienda. Recomendado: cuadrado, min 256x256px"
          : "Imagen de portada. Recomendado: 16:9, min 800x450px"}
      </p>
    </div>
  );
}